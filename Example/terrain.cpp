...
void frame(int width, int height)
{
    constexpr auto MS = R"(
        ...
        void assembleLOD0(vec3 in_vertex[3], vec2 in_uv[3], inout uint vid, inout uint iid)
        {
            for(uint i = 0; i < 3; ++i)
            {
                ...
                rhi_MeshVerticesNV[vid + i].gl_Position = projMat * viewMat * vec4(in_vertex[i], 1);
                rhi_PrimitiveIndicesNV[iid + i] = vid + i;
            }
            vid += 3;
            iid += 3;
        }
        ...
        void main()
        {
            uint meshlet_id = rhi_WorkGroupID.x;
    ...
    static auto module = rhi_create_module_meshlet(nullptr, MS, FS);
    ...
    {
        rhi_pass_t pass = {.module = module, .colors = {{.texture = pass_color, .clear = true,}}, .depth = {.texture = pass_depth, .clear = true, .write = true, .func = GL_LEQUAL,}, .fill_mode = GL_FILL,};
